...
항공우주 문서를 Colab 또는 로컬 Python에서 직접 호출해 쓰는 core-only RAG 런타임입니다. `data/` 폴더의 지원 문서를 자동 수집하고, Qdrant dense/sparse/image 검색, JSON graph-lite 검색, 고정 weighted RRF, Ollama 답변 생성을 제공합니다. 서버/컨테이너 배포, 멀티테넌트 private overlay, 외부 LLM provider, DAT/QACT runtime profile 운영은 이 프로젝트 범위에서 제외합니다.
