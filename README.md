...
기본값은 배포 재현성을 위해 위 5개 파일을 엄격히 요구합니다. 스마트팜 ingest처럼 폴더 안의 지원 파일을 모두 훑으려면:

```powershell
.venv\Scripts\python -m aerospace_rag.ingest --data-dir data --no-strict-expected
```
...
The Python-only runtime now mirrors the SmartFarm internals more closely while intentionally excluding FastAPI and Docker:

- Qdrant stores SmartFarm-style canonical payload keys such as `canonical_doc_id`, `canonical_chunk_id`, `doc_id`, and `chunk_id`.
- If `qdrant-client` is unavailable, the same vector channel falls back to a local JSON vector store so the core runtime remains executable before dependency installation.
...
- Ingest includes a dependency-light SmartFarm-style document parser for PDF/XLSX/image/text files.
- Graph build now performs deterministic entity/relation extraction and writes a graph helper index with direct entity matches plus bounded relation-neighbor expansion.
- Fusion uses DAT-style query segmentation and channel diagnostics instead of a single fixed weight, and can read SmartFarm runtime profile keys such as `vector_dense_text`, `vector_sparse`, and `graph`.
...
Docker 없이도 edge-sync style artifact manifest를 만들 수 있습니다:

```powershell
.venv\Scripts\python -m aerospace_rag.artifact_export --index-dir data/index --output-dir data/index/export
.venv\Scripts\python -m aerospace_rag.artifact_import --manifest data/index/export/artifact_manifest.json --index-dir data/index
```
...
FastAPI endpoint 없이도 SmartFarm의 private overlay core를 로컬 SQLite로 사용할 수 있습니다:

```powershell
.venv\Scripts\python -m aerospace_rag.private_ingest "고객사 전용 H3 검토 메모" --farm-id tenant-a
.venv\Scripts\python -m aerospace_rag.query "H3 검토 메모는?" --include-private --farm-id tenant-a --debug
```

`PRIVATE_LLM_POLICY=local_only` 기본값에서는 private 근거가 포함된 질의가 remote `openai_compatible` provider로 나가지 않도록 extractive fallback을 사용합니다.
