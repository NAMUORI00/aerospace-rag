...
`ask()`는 이미 만들어진 로컬 인덱스를 읽어 질문을 검색합니다. Qdrant dense text, Qdrant sparse, Qdrant dense image, BM25, graph-lite 결과를 모은 뒤 weighted RRF로 합치고, 상위 chunk를 Transformers, Ollama, 또는 명시적 extractive debug provider에 전달해 답변과 근거를 반환합니다.
...
- `ollama`: 로컬/클라우드 Ollama API를 쓰는 선택 경로
- `transformers`: Colab GPU 안에서 Gemma 4 모델을 직접 로드하는 기본 노트북 경로
- `extractive`: LLM 없이 검색 결과만 확인하는 명시적 debug 경로

Transformers 또는 Ollama 호출이 실패해도 자동으로 extractive 답변으로 우회하지 않습니다. 이 정책은 실패를 숨기지 않고, 실제 생성 모델이 준비되지 않은 상태를 명확히 드러내기 위한 것입니다.
...
Colab 흐름의 기본 의도는 엄격합니다. extraction은 Transformers backend를 사용하고, `google/gemma-4-E4B-it`를 Colab T4에서 4-bit로 직접 로드합니다. 자동 local fallback을 켜지 않으므로 실제 LLM extraction 경로가 실패하면 인덱싱도 명확히 실패합니다.
...
- `aerospace_rag.generation`: Transformers, Ollama, extractive provider
